...
CREATE OR REPLACE FUNCTION rq_HC_L1 (center_id integer, radius FLOAT8) RETURNS SETOF genericQuery AS $$
BEGIN
...
CREATE OR REPLACE FUNCTION rq_HC_LInf (center_id integer, radius FLOAT8) RETURNS SETOF genericQuery AS $$
BEGIN
	...
	DECLARE fprev_id integer; fnext_id integer; distance float8; n_inserted integer := 0; border float8;
		BEGIN
			...
			LOOP
				...
				fprev_id = fnext_id;
			END LOOP;
		RAISE NOTICE 'Inserting distances';
...
CREATE OR REPLACE FUNCTION createHCFocusBaseLinf(num integer)
	RETURNS VOID AS $$
	...
	DECLARE fprev_id integer; fnext_id integer; distance float8; n_inserted integer := 0; border float8;
		BEGIN
			...
			LIMIT 1;
			LOOP
				select S2.cod, cube(S1.feature) <=> cube(S2.feature) as dist into fnext_id, distance from HC_TABLE S1, HC_TABLE S2 -- BASE CRIADA UTILIZANDO A DISTANCIA LINF
				where S1.cod = fprev_id AND S2.is_focus = 'False'
				order by dist DESC
				LIMIT 1;
	...
	INSERT INTO hc_f_base (id_focus, id_feature, dist_l1, dist_l2, dist_linf) (
		select S1.cod, S2.cod, cube(S1.feature) <#> cube(S2.feature), cube(S1.feature) <->  cube(S2.feature), 0
...
CREATE OR REPLACE FUNCTION rangeOmniHCL1F1 (center_id integer, radius FLOAT8) RETURNS SETOF genericQuery AS $$
	DECLARE rec_id RECORD; feature_aux FLOAT8[]; distance FLOAT8; dist_fc FLOAT8; cand INTEGER := 0;
	BEGIN
		
		select T1.feature into feature_aux FROM HC_TABLE T1 where T1.cod = center_id;
		select dist_l1 INTO dist_fc from HC_F_BASE where id_feature = center_id;
		
		
		FOR rec_id IN SELECT id_feature from HC_F_BASE WHERE ((dist_l1 < (radius + dist_fc)) AND (dist_l1 > dist_fc - radius)) LOOP
				cand = cand + 1;
...
CREATE OR REPLACE FUNCTION rangeOmniHCL1F2 (center_id integer, radius FLOAT8) RETURNS SETOF genericQuery AS $$
	DECLARE rec_id RECORD; feature_aux FLOAT8[]; distance FLOAT8; dist_fc FLOAT8[]; cand INTEGER := 0;
	BEGIN
		
		select T1.feature into feature_aux FROM HC_TABLE T1 where T1.cod = center_id;
		dist_fc = array(select distinct dist_l1 from HC_F_BASE where id_feature = center_id);
		--RAISE NOTICE 'dist_fc %  dist_fc2 %', dist_fc[1], dist_fc[2];
		
		FOR rec_id IN SELECT id_feature from HC_F_BASE WHERE ((dist_l1 < (radius + dist_fc[1])) AND (dist_l1 > dist_fc[1] - radius)) INTERSECT
					(SELECT id_feature from HC_F_BASE WHERE ((dist_l1 < (radius + dist_fc[2])) AND (dist_l1 > dist_fc[2] - radius))) LOOP
				cand = cand + 1;	
				select (cube(feature_aux) <#> cube(feature)) INTO distance FROM HC_TABLE T1 WHERE rec_id.id_feature = T1.cod ;
				if (distance < radius) then
					RETURN NEXT (rec_id.id_feature, distance);			       
...
CREATE OR REPLACE FUNCTION rangeOmniHCL1F3 (center_id integer, radius FLOAT8) RETURNS SETOF genericQuery AS $$
	DECLARE rec_id RECORD; feature_aux FLOAT8[]; distance FLOAT8; dist_fc FLOAT8[]; cand INTEGER := 0;
	BEGIN
		...
		FOR rec_id IN SELECT id_feature from HC_F_BASE WHERE ((dist_l1 < (radius + dist_fc[1])) AND (dist_l1 > dist_fc[1] - radius)) INTERSECT
					(SELECT id_feature from HC_F_BASE WHERE ((dist_l1 < (radius + dist_fc[2])) AND (dist_l1 > dist_fc[2] - radius))) INTERSECT
					(SELECT id_feature from HC_F_BASE WHERE ((dist_l1 < (radius + dist_fc[3])) AND (dist_l1 > dist_fc[3] - radius))) LOOP
...
CREATE OR REPLACE FUNCTION rangeOmniHCL1F4 (center_id integer, radius FLOAT8) RETURNS SETOF genericQuery AS $$
	DECLARE rec_id RECORD;feature_aux FLOAT8[]; distance FLOAT8; dist_fc FLOAT8[]; cand INTEGER := 0;
	BEGIN
		...
		FOR rec_id IN SELECT id_feature from HC_F_BASE WHERE ((dist_l1 < (radius + dist_fc[1])) AND (dist_l1 > dist_fc[1] - radius)) INTERSECT
					(SELECT id_feature from HC_F_BASE WHERE ((dist_l1 < (radius + dist_fc[2])) AND (dist_l1 > dist_fc[2] - radius))) INTERSECT
					(SELECT id_feature from HC_F_BASE WHERE ((dist_l1 < (radius + dist_fc[3])) AND (dist_l1 > dist_fc[3] - radius))) INTERSECT
					(SELECT id_feature from HC_F_BASE WHERE ((dist_l1 < (radius + dist_fc[4])) AND (dist_l1 > dist_fc[4] - radius))) LOOP
...
CREATE OR REPLACE FUNCTION rangeOmniHCL1F5 (center_id integer, radius FLOAT8) RETURNS SETOF genericQuery AS $$
	DECLARE rec_id RECORD;feature_aux FLOAT8[]; distance FLOAT8; dist_fc FLOAT8[]; cand INTEGER := 0;
	BEGIN
		...
		FOR rec_id IN SELECT id_feature from HC_F_BASE WHERE ((dist_l1 < (radius + dist_fc[1])) AND (dist_l1 > dist_fc[1] - radius)) INTERSECT
					(SELECT id_feature from HC_F_BASE WHERE ((dist_l1 < (radius + dist_fc[2])) AND (dist_l1 > dist_fc[2] - radius))) INTERSECT
					(SELECT id_feature from HC_F_BASE WHERE ((dist_l1 < (radius + dist_fc[3])) AND (dist_l1 > dist_fc[3] - radius))) INTERSECT
					(SELECT id_feature from HC_F_BASE WHERE ((dist_l1 < (radius + dist_fc[4])) AND (dist_l1 > dist_fc[4] - radius))) INTERSECT
					(SELECT id_feature from HC_F_BASE WHERE ((dist_l1 < (radius + dist_fc[5])) AND (dist_l1 > dist_fc[5] - radius))) LOOP
